...
export default class BookingService extends Service {
    ...
    public async createBooking(input: {
        ...
        if (!professional) return this.responseData(404, true, "Professional was not found.");

        if (!professional.availability) return this.responseData(400, true, "Professional is unavailable.");

        let serviceExists = await this.serviceRepo.find({where: {id: In(serviceIds), professionalId: professionalId}});
        if (!serviceExists || serviceExists.length < serviceIds.length) return this.responseData(HttpStatus.BAD_REQUEST, true, `Invalid services.`, serviceExists);

        // --- Step 1: Validate input ---
        if (startDateTime >= endDateTime) return this.responseData(400, true, 'Start time must be before end time');
        ...
        try {
            ...
            return this.responseData(201, false, "Professional was booked successfully", data)
        } catch (error) {
    ...
    async updateBooking(
        ...
        try {
            const data = await AppDataSource.transaction(async (manager) => {
                ...
                if (updates.status && ['cancelled', 'no_show'].includes(updates.status)) {
            ...
            return this.responseData(200, false, "Booking was updated successfully", data)
    ...
    public async getProBooking(bookingId: string, proId: string) {
        try {
            ...
            return this.responseData(200, false, "Booking has been retrieved successfully", booking);
        } catch (error) {
            return this.handleTypeormError(error);
        }
    }
    ...
    public async acceptBooking(bookingId: string, proId: string) {
        try {
            ...
            if (booking.status !== BookingStatus.PENDING) return this.responseData(400, true, `This booking can't be accepted`);
            ...
            return this.responseData(200, false, "Booking has been accepted successfully", {
                ...updatedBooking,
    ...
    public async rejectBooking(bookingId: string, proId: string) {
        try {
            ...
            if (!booking) return this.responseData(404, true, "Booking was not found");
            if (booking.status !== BookingStatus.PENDING) return this.responseData(400, true, `This booking can't be rejected`);
            ...
            return this.responseData(200, false, "Booking has been rejected successfully", {
                ...updatedBooking,
                professional: undefined
            });
        } catch (error) {
    ...
    public async getProBookings(proId: string, page: number, limit: number) {
        try {
            ...
            return this.responseData(200, false, "Bookings have been retrieved successfully", data)
        } catch (error) {
            return this.handleTypeormError(error);
        }
    }
